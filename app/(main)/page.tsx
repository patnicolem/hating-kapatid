  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const [invitesResponse, friendsResponse] = await Promise.all([
          fetch("/api/invitations"),
          fetch("/api/friends"),
        ]);

        for (const response of [invitesResponse, friendsResponse]) {
          if (response.status === 401) {
            window.location.assign("/login");
            return;
          }
        ...
        const invitationsData = await invitesResponse.json();
      ...
      <div className="mt-8">
        ...
        {groups.length === 0 ? (
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {groups.map((group) => {
              const balance = computeMemberNetBalance(
                group.expenses,
                user?.id ?? ""
              );
